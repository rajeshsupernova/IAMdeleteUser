        ...
        foreach ($iamUsers['Users'] as $iamUser) {
                if (isset($iamUser['UserName']) && strtolower($apiUserEmail) === strtolower($iamUser['UserName'])) {
                        ...
                        try {
                                ...
                                try {
                                        $loginProfile = $iamClient->getLoginProfile([
                                                'UserName' => $iamUser['UserName'],
                                        ]);

                                        // If a login profile exists, delete it
                                        $iamClient->deleteLoginProfile([
                                                'UserName' => $iamUser['UserName'],
                                        ]);

                                        echo "Login profile for user $username deleted successfully.\n";
                                } catch (\Aws\Exception\AwsException $e) {
                                ...
                                $listMFA = $iamClient->listMFADevices([
                                        'UserName' => $iamUser['UserName'],
                                ]);
